...
(59 claims, 0 violations). The Kuramoto--Sivashinsky case is the primary
...
14 false claims of the replica plus 30 of its correct claims,
$k = 1$ is violated in 2 cases --- the fitted
...
59 claims: 51 exact and 8 strict
false claims, all of them sub-support claims of the same class. \textbf{In all
8, the missing term sits below $X$}: margins from
1.68$\times$ to 4.40$\times$, and the declared
smallest detectable coefficient of the missing term (median
$1.4 \times 10^{-6}$) exceeds its true value (median $4.8 \times 10^{-7}$) --- the bound
says, correctly, that the term was not resolvable. No verdict changed; median
$X$ is 3.0\%. Verdict \texttt{CONFIRMED} by the
...
amplitude-extrapolation gate. The 8 strict false claims
...
stated scope. Zero violations in 59 claims bounds the violation
rate at 5.1\% (rule of three): one sealed replication,
...
(a)~The 14 sub-support false claims of the sealed replica:
...
($k=1$: 2 violations; $k=2$: none).
(b)~The 8 sub-support claims of the second sealed panel,
scored blind with $k=2$: $f$ against $X$, all below the diagonal (margins
1.68--4.40$\times$), with the $X$ of the
51 exact claims as a marginal strip.}
