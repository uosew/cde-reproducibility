...
artifacts (frozen reference commit \texttt{8ecc3f85}, protocol v1.1);
...
deviation, with maximum coefficient error 1.11\% across all
systems, noise levels and two independent seeds (7 and 11), while maintaining \textbf{zero false
discoveries in 120 null-control runs} (temporal-shuffle and
phase-randomized surrogates). A later sealed replica of the same ladder on 603
thermal-camera cases (423 opportunities to claim falsely) measured 14
false claims (3.3\%), all sub-support claims on a term carrying
...
(59 and 50 claims on two sealed panels, 0 violations). The Kuramoto--Sivashinsky case is the primary
...
residuals $\approx 0.80$ --- and its remedy, a residual-based
...
resolutions (9 temporal and 48 spatial configurations, all
...
finite differences to $1.1 \times 10^{-6}$ relative error.
...
\textbf{356/356 numeric fields bit-identical} to the 3.13.10
canonical run (operator validation campaign: 20/20 fields
bit-identical).
...
discoveries in 20 nulls (preflight $r_{GT}$: $4.6 \times 10^{-5}$ and
$4.7 \times 10^{-5}$).
...
\midrule
Allen--Cahn & $u_t = 0.05 u_{xx} + u - u^3$ & bistable & ETDRK4 & $1.5 \times 10^{-4}$ \\
Fisher--KPP & $u_t = 0.05 u_{xx} + u - u^2$ & reaction--diffusion & ETDRK4 & $1.0 \times 10^{-4}$ \\
Burgers & $u_t = -u u_x + 0.08 u_{xx}$ & nonlinear advection & ETDRK4 (+FD4/RK45) & $4.6 \times 10^{-5}$ \\
KdV~\cite{zabusky1965} & $u_t = -u u_x - 0.0484\, u_{xxx}$ & dispersive & complex-L ETDRK4 & $8.6 \times 10^{-5}$ \\
Kuramoto--Sivashinsky~\cite{kuramoto1978,sivashinsky1977} & $u_t = -u u_x - u_{xx} - u_{xxxx}$ & chaos, $L = 32\pi$ & ETDRK4 & $8.7 \times 10^{-6}$ \\
\bottomrule
\end{tabular}}
\end{center}
...
two independent seeds (7 and 11): exact support recovery, maximum coefficient error
1.11\%, maximum claimed-fit residual $4.1 \times 10^{-2}$, and \textbf{0 false
discoveries in 120 null runs}. Figure~\ref{fig:coeferr} traces
...
\begin{equation*}
u_t = -1.0000\, u u_x -1.0000\, u_{xx} -1.0000\, u_{xxxx}
\qquad (\sigma = 0),
\end{equation*}
...
coefficients remain $(-0.9992,\ -0.9981,\
-0.9949)$. This sign-structure discrimination under chaos is the
...
\midrule
0\% & $u_{xx}$, $u_{xxxx}$, $u u_x$ & $u_{xx}$: $-1.0000$, $u_{xxxx}$: $-1.0000$, $u u_x$: $-1.0000$ & 0.00\% & $7.9 \times 10^{-6}$ & PASS \\
1\% & $u_{xx}$, $u_{xxxx}$, $u u_x$ & $u_{xx}$: $-1.0000$, $u_{xxxx}$: $-1.0000$, $u u_x$: $-1.0000$ & 0.00\% & $1.3 \times 10^{-3}$ & PASS \\
2\% & $u_{xx}$, $u_{xxxx}$, $u u_x$ & $u_{xx}$: $-1.0000$, $u_{xxxx}$: $-1.0005$, $u u_x$: $-0.9999$ & 0.05\% & $2.6 \times 10^{-3}$ & PASS \\
5\% & $u_{xx}$, $u_{xxxx}$, $u u_x$ & $u_{xx}$: $-0.9991$, $u_{xxxx}$: $-0.9990$, $u u_x$: $-0.9992$ & 0.10\% & $6.4 \times 10^{-3}$ & PASS \\
10\% & $u_{xx}$, $u_{xxxx}$, $u u_x$ & $u_{xx}$: $-0.9981$, $u_{xxxx}$: $-0.9949$, $u u_x$: $-0.9992$ & 0.51\% & $1.3 \times 10^{-2}$ & PASS \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
Allen–Cahn & $u$, $u^3$, $u_{xx}$ & 1.11\% & $4.1 \times 10^{-2}$ \\
Fisher–KPP & $u$, $u^2$, $u_{xx}$ & 0.65\% & $5.3 \times 10^{-3}$ \\
Burgers & $u_{xx}$, $u u_x$ & 0.12\% & $1.5 \times 10^{-2}$ \\
KdV & $u_{xxx}$, $u u_x$ & 0.31\% & $2.6 \times 10^{-2}$ \\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{Setup (full disclosure).} PySINDy~\cite{desilva2020,kaptanoglu2022} v2.1.0: \texttt{CustomLibrary} $\{u, u^2, u^3\}$ wrapped in \texttt{WeakPDELibrary} (derivative order 3; 4 for KS), $K = 200$ weak subdomains, no bias term, PySINDy's STLSQ with ridge $\alpha = 10^{-5}$, threshold grid $\{0.01, 0.02, 0.05, 0.1, 0.2, 0.5\}$ with \textbf{oracle selection} per (system, noise level) --- the best threshold judged against ground truth, an advantage the frozen CDE protocol never receives. Feature names mapped (\texttt{x0} $= u$); the library is PySINDy's natural weak set including all cross terms $f(u)\,\partial_x^k u$, a superset of ours. \emph{Exact support} = all true terms nonzero and every other term zero. On nulls, the $\sigma = 0$-selected threshold is used (fixed policy).

\paragraph{Recovery.} The baseline achieves exact support in 19/25 (system, $\sigma$) cells versus 25/25 for the frozen CDE protocol (failures: Fisher--KPP at every noise level, KdV at $\sigma \geq 5\%$). Where it recovers (Burgers, KS, Allen--Cahn), its oracle-tuned coefficients are accurate.

\paragraph{Nulls, and the causal decomposition.} Vanilla PySINDy commits a false discovery on \textbf{49/50 null fields} (CDE: 0/120), even though an empty model was \emph{reachable} in its threshold grid on 33/50 of them: STLSQ has no criterion that ever prefers it. We then ran the ablation \textbf{PySINDy + epistemic gate}, transplanting the v1.1 residual gate (proxy: $\sqrt{1 - R^2}$ of PySINDy's own weak regression, same 5\% level): null FDR drops to \textbf{0/50} --- the null-rejection advantage is governance, and it transfers. But the same gate also rejects the baseline's \emph{true} fits at moderate noise (only 14/25 cells remain claimed discoveries; KdV falls at $\sigma \geq 2\%$, Allen--Cahn and Fisher--KPP at $\sigma \geq 5\%$), while \emph{all} 25 CDE cells pass the identical gate. The decomposition is clean: null rejection comes from the epistemic layer; noise-robust discovery power comes from the quality of the validated weak operator. Both are needed, and neither alone explains the paired result (Figure~\ref{fig:gatenull}b).
...
policy PySINDy's nulls were scored with, it claims a model on \textbf{40
of 40 null fields}, selecting a median of 7 of 7 terms; PySINDy claims on
40 of 40. Neither has a criterion that prefers the empty model. Transplanting
the same 5\% residual gate takes the independent implementation to
\textbf{0 of 40}: null residuals run
0.798--0.998 against at most 4.1e-02 on true cells, a
factor 19. Reported against us: allowed the most generous
policy --- choosing, per case, a threshold that abstains where one exists --- its claims fall to
10 of 40. Sparse selection \emph{can} abstain; it has
...
independent optimizer recovers the exact support in \textbf{20 of
20} cells using our weak features, where PySINDy recovered 14 of
20 using its own. With the optimizer family held fixed, the difference is the weak
...
with per-system pre-gate false-discovery rates of 0.0 (Allen--Cahn),
0.3 (Fisher--KPP), 0.4 (Burgers), 0.5 (KdV). Chance
...
support is 0.80, versus $4.1 \times 10^{-2}$ \emph{maximum} for true
...
smooth or quasi-stationary fields yield 0.30--0.50
(Fisher--KPP/Burgers/KdV; external-solver arm:
0.30--0.40), while the chaotic KS field yields
0.00 --- on broadband chaos, the nulls do not even produce stable
...
$r_{GT} =$ $8.7 \times 10^{-6}$). Both decisions were made from the oracle residual
...
ledger before generation) holds 603 thermal-camera-degraded diffusion
fields in nine balanced classes: 268 representable
(true support inside the library), 268 non-representable
(a term outside the library), and 67 nulls. Two nodes ran it blind
...
wrong support; the panel offered 423 of them, against a
...
\textbf{Result: 14 false claims in 423 opportunities}
(3.3\%; the rule-of-three bound the campaign hoped for was
0.71\%). Zero on the 67 nulls, zero on the
268 non-representable cases; 105 correct
claims on 268 representable cases (recall
39\%, median coefficient
error 1.17\%). The two nodes agree on all 100
...
\paragraph{One failure class.} All 14 false claims are in
...
14 it does not see the problem at all. We call this a
...
120 explicit null controls of the PDE campaigns and remain true
...
false-claim rate is 3.3\%, concentrated entirely in sub-support
...
4 of 22 cases --- the fitted model absorbs part of the missing term through
...
59 claims, 8 of them sub-support, with the missing term below
the bound in all 8 (margins 1.68--4.40$\times$)
and no verdict changed.

The projected form of \eqref{eq:bound} then removes the factor. On the same diagnostic it is
violated in 0 of 22 cases at $k = 1$, and on a second sealed panel of 120 cases
it gives 50 claims, 7 sub-support, \textbf{0 violations}
(margins 1.85--4.75$\times$, median 3.49$\times$),
median $X$ 1.4\%, no verdict changed. On that panel the unprojected
bound at $k = 2$ is also sound (margins 1.69--4.15$\times$), so the panel
does not separate the two on soundness. It separates them on where the factor comes from:
$\|\tilde a_t\| / \|a_t\|$ has median 0.46 on these cases, i.e.\ about
...
gate. The 15 strict false claims of the two panels stay
...
50 claims bounds the violation rate at 6.0\% (rule of
...
22 sub-support claims with open truth: the unprojected bound $\|r\|/\|a_t\|$ at
$k = 1$ is violated in 4 cases (open squares below the line), the projected bound
$\|r\|/\|\tilde a_t\|$ in none. (b)~The two sealed panels, scored blind: 8
sub-support claims under the unprojected bound at $k = 2$ and 7 under the
...
headline runs use two independent seeds (7 and 11). Two preregistered 20-seed campaigns
...
discoveries in 120 null controls. That last figure is a property of
the null controls, not of the ladder: a sealed replica on 603 cases measured a
3.3\% false-claim rate, entirely from sub-support claims on a term
...
(4660 Python files; 809 flagged for review,
2 confirmed-and-fixed) and a mandatory runtime canary followed.
...
\midrule
V7 (py3.13) & 7 & 3.13.10 & 2.5.1 & PASS & \texttt{a6f14e9ea5cabaf6} \\
V8 (py3.13) & 7 & 3.13.10 & 2.5.1 & PASS & \texttt{e5662ad857671b4d} \\
V8 (py3.12) & 7 & 3.12.12 & 2.5.1 & PASS & \texttt{e1704f0b5b24e45e} \\
External solver & 7 & 3.13.10 & 2.5.1 & PASS & \texttt{26439b27085048b5} \\
V9 KS (py3.13) & 7 & 3.13.10 & 2.5.1 & PASS & \texttt{9b4a119d6ed677d2} \\
V8 (seed 11) & 11 & 3.13.10 & 2.5.1 & PASS & \texttt{b359f86811322338} \\
V9 KS (seed 11) & 11 & 3.13.10 & 2.5.1 & PASS & \texttt{ed42bcc6dd6c4588} \\
V13 sealed replica (scored) & 700000+ & 3.13.10 & 2.5.1 & PASS & \texttt{04d08889eddb7fa2} \\
Blind-4 resolution bound & 720000+ & 3.13.10 & 2.5.1 & PASS & \texttt{945dc846e72f94dd} \\
Blind-5 projected bound & 740000+ & 3.13.10 & 2.5.1 & PASS & \texttt{2d70321320fe5d7b} \\
Independent optimizer (Julia) & 7 & 3.13.10 & 2.5.1 & PASS & \texttt{4714835fe5920de2} \\
\bottomrule
\end{tabular}
\end{center}
Frozen reference commit: \texttt{8ecc3f85} (protocol v1.1). This note and
...
$\sigma$ & Support & Coefficients & Max err & Residual & Gate \\
\midrule
0\% & $u$, $u^3$, $u_{xx}$ & $u$: $+1.0000$, $u^3$: $-1.0000$, $u_{xx}$: $+0.0500$ & 0.01\% & $1.4 \times 10^{-4}$ & PASS \\
1\% & $u$, $u^3$, $u_{xx}$ & $u$: $+1.0002$, $u^3$: $-0.9999$, $u_{xx}$: $+0.0500$ & 0.08\% & $5.3 \times 10^{-3}$ & PASS \\
2\% & $u$, $u^3$, $u_{xx}$ & $u$: $+0.9999$, $u^3$: $-0.9995$, $u_{xx}$: $+0.0500$ & 0.05\% & $7.8 \times 10^{-3}$ & PASS \\
5\% & $u$, $u^3$, $u_{xx}$ & $u$: $+1.0026$, $u^3$: $-0.9996$, $u_{xx}$: $+0.0500$ & 0.26\% & $2.2 \times 10^{-2}$ & PASS \\
10\% & $u$, $u^3$, $u_{xx}$ & $u$: $+1.0111$, $u^3$: $-0.9998$, $u_{xx}$: $+0.0503$ & 1.11\% & $4.1 \times 10^{-2}$ & PASS \\
\bottomrule
\end{tabular}\end{center}

\subsection*{Fisher–KPP}
\begin{center}\small\begin{tabular}{llp{5.2cm}lll}
\toprule
$\sigma$ & Support & Coefficients & Max err & Residual & Gate \\
\midrule
0\% & $u$, $u^2$, $u_{xx}$ & $u$: $+1.0000$, $u^2$: $-1.0000$, $u_{xx}$: $+0.0500$ & 0.01\% & $9.2 \times 10^{-5}$ & PASS \\
1\% & $u$, $u^2$, $u_{xx}$ & $u$: $+0.9999$, $u^2$: $-0.9999$, $u_{xx}$: $+0.0501$ & 0.13\% & $7.2 \times 10^{-4}$ & PASS \\
2\% & $u$, $u^2$, $u_{xx}$ & $u$: $+1.0003$, $u^2$: $-1.0003$, $u_{xx}$: $+0.0499$ & 0.13\% & $1.2 \times 10^{-3}$ & PASS \\
5\% & $u$, $u^2$, $u_{xx}$ & $u$: $+1.0000$, $u^2$: $-0.9999$, $u_{xx}$: $+0.0503$ & 0.53\% & $2.9 \times 10^{-3}$ & PASS \\
10\% & $u$, $u^2$, $u_{xx}$ & $u$: $+1.0035$, $u^2$: $-1.0039$, $u_{xx}$: $+0.0503$ & 0.65\% & $5.3 \times 10^{-3}$ & PASS \\
\bottomrule
\end{tabular}\end{center}
...
$\sigma$ & Support & Coefficients & Max err & Residual & Gate \\
\midrule
0\% & $u_{xx}$, $u u_x$ & $u_{xx}$: $+0.0800$, $u u_x$: $-1.0000$ & 0.01\% & $2.2 \times 10^{-5}$ & PASS \\
1\% & $u_{xx}$, $u u_x$ & $u_{xx}$: $+0.0800$, $u u_x$: $-1.0000$ & 0.00\% & $1.9 \times 10^{-3}$ & PASS \\
2\% & $u_{xx}$, $u u_x$ & $u_{xx}$: $+0.0800$, $u u_x$: $-0.9999$ & 0.02\% & $2.9 \times 10^{-3}$ & PASS \\
5\% & $u_{xx}$, $u u_x$ & $u_{xx}$: $+0.0801$, $u u_x$: $-0.9996$ & 0.14\% & $8.3 \times 10^{-3}$ & PASS \\
10\% & $u_{xx}$, $u u_x$ & $u_{xx}$: $+0.0799$, $u u_x$: $-0.9988$ & 0.12\% & $1.5 \times 10^{-2}$ & PASS \\
\bottomrule
\end{tabular}\end{center}
...
$\sigma$ & Support & Coefficients & Max err & Residual & Gate \\
\midrule
0\% & $u_{xxx}$, $u u_x$ & $u_{xxx}$: $-0.0484$, $u u_x$: $-1.0000$ & 0.00\% & $8.3 \times 10^{-5}$ & PASS \\
1\% & $u_{xxx}$, $u u_x$ & $u_{xxx}$: $-0.0484$, $u u_x$: $-1.0001$ & 0.03\% & $3.0 \times 10^{-3}$ & PASS \\
2\% & $u_{xxx}$, $u u_x$ & $u_{xxx}$: $-0.0484$, $u u_x$: $-0.9990$ & 0.10\% & $6.2 \times 10^{-3}$ & PASS \\
5\% & $u_{xxx}$, $u u_x$ & $u_{xxx}$: $-0.0485$, $u u_x$: $-1.0004$ & 0.16\% & $1.4 \times 10^{-2}$ & PASS \\
10\% & $u_{xxx}$, $u u_x$ & $u_{xxx}$: $-0.0485$, $u u_x$: $-1.0031$ & 0.31\% & $2.6 \times 10^{-2}$ & PASS \\
\bottomrule
\end{tabular}\end{center}
