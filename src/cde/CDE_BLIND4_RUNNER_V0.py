...
def main():
    ...
    kstar = json.loads((ART / "cde_risoluzione_claim_out" / "esito_stage1.json").read_text())["esito"]["k_star"]
    assert kstar is not None, "Stage 1 non ha scelto k*: nessuna annotazione possibile"
    k = float(kstar); pub = json.loads((OUT / "public_meta.json").read_text())
    ...
    for cid, v in verd.items():
        ...
        if v["verdict"] == "CLAIM":
            ...
            ann[cid]["risoluzione"] = RIS.annota(A, b, v["support"], v["coefficients"], k)
    (OUT / "verdicts_annotati.json").write_text(json.dumps({"k_star": k, "verdetti": ann}, indent=1, default=str))
