...
PAPER = BASE.parent.parent / "paper"; OUT = ART / "cde_blind5_out"
def _L(n, f):
...
def valuta(ann, truth, contributo):
    """ann: verdetti annotati; truth; contributo(cid, t) -> |c_t vero| (blind-5: coefficienti). Esito prereg blind-5 §3."""
    ...
    for c in claims:
        ...
        if mancanti and not spurii:
            sotto += 1
            cv = {t: contributo(c, t) for t in mancanti}
            cmin = v["risoluzione"]["c_min"]
            if all(cv[t] < cmin[t] for t in mancanti): coperte += 1
            else: viol.append({"case": c, "cls": tr["cls"], "c_vero": cv, "c_min_v2": {t: cmin[t] for t in mancanti}})
...
def mutation_test():
    ...
    def V(verdict, sup, cmin=1e-6):
        return {"verdict": verdict, "support": sup, "risoluzione": {"X": 0.01, "c_min": {t: cmin for t in ("div_vv_x", "v", "v_xx", "v2", "vv_x", "v_x", "const")}}}
    ok = {}
    tr = {f"c{i}": T(["v", "v_xx"]) for i in range(30)}; tr["s"] = T(["v", "v_xx", "div_vv_x"])
    an = {f"c{i}": V("CLAIM", ["v", "v_xx"]) for i in range(30)}; an["s"] = V("CLAIM", ["v", "v_xx"], cmin=1e-6)
    ok["31 claim, coefficiente mancante sotto c_min -> CONFIRMED"] = valuta(an, tr, lambda c, t: 5e-7)["verdetto"] == "CONFIRMED"
    ok["coefficiente mancante >= c_min -> REJECTED"] = valuta(an, tr, lambda c, t: 2e-6)["verdetto"] == "REJECTED"
    an2 = {k: v for k, v in list(an.items())[:20]}; tr2 = {k: tr[k] for k in an2}
    ok["< 30 claim -> INCONCLUSIVE"] = valuta(an2, tr2, lambda c, t: 5e-7)["verdetto"] == "INCONCLUSIVE"
    an3 = dict(an); an3["c0"] = dict(an["c0"], verdict_originale="NOT_IDENTIFIABLE")
    ok["verdetto cambiato dall'annotazione -> REJECTED"] = valuta(an3, tr, lambda c, t: 5e-7)["verdetto"] == "REJECTED"
    an4 = dict(an); an4["c1"] = V("CLAIM", ["v", "v_xx", "v2"]); e = valuta(an4, tr, lambda c, t: 5e-7)
    ok["termine spurio = falsa stretta, non sotto-supporto"] = e["false_strette"] == 2 and e["sotto_supporto"] == 1
    return ok
...
def main():
    ...
    runner = (BASE / "CDE_BLIND5_RUNNER_V0.py").read_text() + (BASE / "CDE_V13_BLIND_DISCOVERER_V0.py").read_text()
    ...
    cache = {}
    def contributo(c, t):                       # blind-5: il COEFFICIENTE vero, confrontato con c_min(t)
        return abs(RIS.coefficienti_veri(truth[c]["model"])[t])
    e = valuta(ann, truth, contributo)
    print(f"\n== BLIND-5 (limite proiettato k=1): {len(truth)} casi, k*={d['k_star']}, sigillo intatto, runner cieco: {not sosp} ==")
